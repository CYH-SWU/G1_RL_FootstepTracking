...
print(f"当前骨盆俯仰角 (Pitch): {np.degrees(euler[1]):.2f}°")
...
# 内部点：中心差分
grad_x[1:-1, :] = (height_map[2:, :] - height_map[:-2, :]) / (2.0 * res)
# 边界：单侧差分
grad_x[0, :] = (height_map[1, :] - height_map[0, :]) / res
grad_x[-1, :] = (height_map[-1, :] - height_map[-2, :]) / res

# Y方向同理
grad_y[:, 1:-1] = (height_map[:, 2:] - height_map[:, :-2]) / (2.0 * res)
grad_y[:, 0] = (height_map[:, 1] - height_map[:, 0]) / res
grad_y[:, -1] = (height_map[:, -1] - height_map[:, -2]) / res

slope_map = np.arctan(np.sqrt(grad_x**2 + grad_y**2)) * 180.0 / np.pi
# 处理可能的 nan（如果高度图有 nan，grad 会 nan）
...
print(f"坡度统计: min={slope_map.min():.2f}°, max={slope_map.max():.2f}°, mean={slope_map.mean():.2f}°, std={slope_map.std():.2f}°")
