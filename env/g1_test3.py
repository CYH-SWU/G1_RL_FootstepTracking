...
class G1TerrainEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.right_clock, self.left_clock = create_phase_reward(
        ...
        self.right_vel_fn = self.right_clock[1]
        self.left_vel_fn = self.left_clock[1]
        self.right_frc_fn = self.right_clock[0]
        self.left_frc_fn = self.left_clock[0]

        def wrap(clock_fn):
            return lambda phase: clock_fn(phase * self.total_duration / self.control_dt)

        self.left_vel_fn_wrapped = wrap(self.left_vel_fn)
        self.right_vel_fn_wrapped = wrap(self.right_vel_fn)
        self.left_frc_fn_wrapped = wrap(self.left_frc_fn)
        self.right_frc_fn_wrapped = wrap(self.right_frc_fn)
    ...
    def _generate_step_sequence(self, mode, num_steps=20, step_height=0.0):
        ...
        if mode == WalkModes.CURVED:
            ...
            total_angle = (num_steps - 1) * self.step_length / R
            # 增加随机扰动（可选）
            ...
            for i in range(1, num_steps):
                theta_i = i * dtheta
                radius_offset = ((-1) ** i) * (self.step_width / 2)
                R_i = R + radius_offset
    ...
    def _compute_reward(self, action):
        ...
        if is_stand:
            ...
            r_frc = calc_foot_frc_clock_reward(
                left_force, right_force, 
                self.phase, max_force,
                clock_left=self.left_frc_fn_wrapped, 
                clock_right=self.right_frc_fn_wrapped
            )
            r_vel = calc_foot_vel_clock_reward(
                left_vel, right_vel, 
                self.phase, self.max_foot_vel,
                clock_left=self.left_vel_fn_wrapped,
                clock_right=self.right_vel_fn_wrapped)
