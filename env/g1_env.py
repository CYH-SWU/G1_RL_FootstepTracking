"""
完全模仿 LHW 的步点跟踪环境
- 使用 LHW 的六种模式：STANDING, CURVED, FORWARD, BACKWARD, INPLACE, LATERAL
- 预生成步点序列，采用预定义踏脚石（box）并更新位置
- 观测：未来两步步点（骨盆坐标系），基础状态 + 特权信息（非对称）
- 动作空间 12 维（移除腰部控制）
- 控制频率 100Hz，物理步长 0.005s
- 步态参数：总周期 1.1s，摆动 0.5s，支撑 0.6s，步长 0.3m，步宽 0.237m
- 课程学习：台阶高度随 difficulty 线性增加，最大 0.1m
"""
