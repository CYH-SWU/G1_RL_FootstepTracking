...
class G1TerrainEnv(gym.Env):
    ...
    def _get_critic_obs(self):
        ...
        # 特权信息：足底力（法向）
        left_force = max(0, self.data.cfrc_ext[self.left_foot_id][2])
        right_force = max(0, self.data.cfrc_ext[self.right_foot_id][2])
        max_force = sum(self.model.body_mass) * 9.81 * 0.5
        norm_left_frc = min(left_force, max_force) / max_force
        norm_right_frc = min(right_force, max_force) / max_force

        norm_left_frc = 2 * norm_left_frc - 1
        norm_right_frc = 2 * norm_right_frc - 1

        # 特权信息：足底速度
        left_vel = self._get_body_linvel(self.left_foot_id)
        right_vel = self._get_body_linvel(self.right_foot_id)

        norm_left_vel = min(left_vel, self.max_foot_vel) / self.max_foot_vel
        norm_right_vel = min(right_vel, self.max_foot_vel) / self.max_foot_vel

        norm_left_vel = 2 * norm_left_vel - 1
        norm_right_vel = 2 * norm_right_vel - 1
